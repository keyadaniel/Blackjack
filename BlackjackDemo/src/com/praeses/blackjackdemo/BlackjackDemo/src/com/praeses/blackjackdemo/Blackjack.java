...
public class Blackjack {
	...
	public static void main(String[] args) {
		...
		while (playerIsSettingBank) {
			System.out.println(
					"How much money would you like to add to your bank? Bets for each hand typically range from $2 - $500 dollars.");
			try {
		...
		while (playerMoney > 0) {
			...
			if (playerHand.cardsValue() == blackJack && dealerHand.cardsValue() != blackJack) {
			...
			while (true) {
				...
				if (response == hit) {
					...
					// Bust if > 21
					if (playerHand.cardsValue() > blackJack) {
			...
			if ((dealerHand.cardsValue() > blackJack) && endRound == false) {
